...
function Detail({selectedMenu,setwindow,showlist}){
  ...
  const options=async()=>{
    await axios.post('http://127.0.0.1:8000/webKiosk/client/opme/read/',{
    market_name : 'S',
    menu_name : `${selectedMenu.menu_name}`}).then(response=>{setoptions(response.data)})
  }
useEffect(()=>{
  options()
},[])
  ...
  function save_order(){
    ...
    let total_price = order_count*(selectedMenu.price+detail_price1+detail_price2)
  ...
  return (
    <div>
      ...
      <div className="select_menu">
        <div className="menu_intro">
          ...
          <div id="menu_info">
            ...
            <h2><label>가격 </label>{selectedMenu.price}원</h2>
            </div>
          </div>
        </div>
        <div id="menu_price">
            ...
            <div id='select_price'>
              <h2>총  {order_count*(selectedMenu.price+detail_price1+detail_price2)}원</h2>
            </div>
          </div>
      </div>
      {/* 메뉴 옵션 */}
      ...
      {/* 메뉴 주문/취소 버튼 */}
      <footer id='fin'>
        <button onClick={()=>(setwindow(false),set_detail_price1(0),set_detail_price2(0),set_order_count(1))}>취소</button>
